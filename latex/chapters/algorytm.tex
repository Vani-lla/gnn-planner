\Chapter{Problem układania planu lekcji i algorytm jego rozwiązania}\label{chapter:algorytm}
    ...
    \section{Sformułowanie problemu optymalizacyjnego}
        ...
        \begin{samepage}
            Oczekiwanym rezultatem działania algorytmu powinno być pięć zbiorów przypisań $\mathcal{Z}_d$ dla każdego roboczego dnia tygodnia $d \in \left\{1, 2, 3, 4, 5\right\}$. Każde takie przypisanie powinno mieć 5 wartości:\label{fragment:przypisania}
            \[ z_i \triangleq \left(d,h,c,t,s,r\right) \in \mathcal{Z}_d, \quad \begin{cases} h &\in \left\{1, 2, \dots, H\right\}\\ c &\in \mathcal{C}\\ t &\in \mathcal{T}\\s &\in \mathcal{S}\\r &\in \mathcal{R}\\ i \end{cases} \]
            gdzie:
            \begin{itemize}
                \item $\mathcal{C}$ to zbiór klas,
                \item $\mathcal{T}$ to zbiór nauczycieli,
                \item $\mathcal{S}$ to zbiór przedmiotów,
                \item $\mathcal{R}$ to zbiór sal,
                \item $H$ to wartość rozpatrywanego horyzontu czasowego.
            \end{itemize}
        ...
        \begin{enumerate}
            \item \textbf{Tworzenie bloków lekcyjnych} z ograniczeń głównych zdefiniowanych w~\ref{subsubsection:ograniczenie_glowne}:
            \begin{itemize}
            ...
            \end{itemize}
            \item \textbf{Przydział bloków lekcyjnych do poszczególnych dni tygodnia.}
            \begin{itemize}
            ...
            \end{itemize}
            \item \textbf{Tworzenie samych przypisań dla każdego dnia tygodnia osobno.}
        \end{enumerate}
        ...
        \subsection{Ograniczenia}\label{subsection:ograniczenia}
            ...
            \subsubsection{Ograniczenie główne}\label{subsubsection:ograniczenie_glowne}
                ...
                Zbiór takich wymagań $\mathcal{W}$ definije się następująco:
                \[ w_i \triangleq \left(t_i, c_i, s_i, g_i\right) \in \mathcal{W}, \quad \begin{cases} t &\in \mathcal{T}\\c &\in \mathcal{C}\\s &\in \mathcal{S}\\g &\in \mathbb{N}^+\\ i &\in \left\{1, 2, \dots \left|\mathcal{W}\right|\right\} \end{cases}  \]
                gdzie $g$ to liczba wymaganych tygodniowo godzin przedmiotu $s$ przeprowadzonego przez nauczyciela $t$ dla klasy $c$.
        
        \subsection{Dane i szukane}
            Warto zacząć od przedstawienia sposobu reprezentacji wymagań w kodzie zaczynając od wymagań głównych.
            Zaczynając procedurę generowania planu lekcji zaczynamy od pobrania z bazy danych odpowiednich rekordów.
            Do otrzymanych w ten sposób rezultatów należy: $\mathcal{C}, \mathcal{T}, \mathcal{S}, \mathcal{R}, \mathcal{W}$.
            ...
            \[ A = \begin{bmatrix}
                ...
                a_{\left|\mathcal{T}\right|,1} & a_{\left|\mathcal{T}\right|,2} & a_{\left|\mathcal{T}\right|,3} & a_{\left|\mathcal{T}\right|,4} & a_{\left|\mathcal{T}\right|,5} \\
            \end{bmatrix}, \quad \forall t \in \left\{1, 2, \dots, \left|\mathcal{T}\right|\right\}, \forall d \in \left\{1, 2, 3, 4, 5\right\}: a_{t, d} \in \left\{0, 1\right\} \]
            W celu tworzenia bloków mamy także dostęp do zbioru bloków przedmiotów $\mathcal{B}$:\label{subject_blocks}
            \[
                \left(\vec{S_b},\vec{C_b}, \vec{G_b}, \verb|is_aggregating|, \verb|max_weekly|\right) \in \mathcal{B}, \quad \begin{cases}
                    \vec{S_b} &\subset \mathcal{S} \\
                    \vec{C_b} &\subseteq  \mathcal{C} \\
                    \vec{G_b} &\in \mathbb{N}^{\left|\vec{S_b}\right|} \\
                    \verb|is_aggregating| &\in \left\{0, 1\right\} \\
                    \verb|max_weekly| & \in \mathbb{N}
                \end{cases}
            \]
            gdzie
            \begin{itemize}
                \item $\vec{S_b}$ to podzbiór wszystkich przedmitów, których dotyczy blok $b$,
                \item $\vec{C_b}$ to podzbiór wszystkich klas, których dotyczy blok $b$,
                \item $\vec{G_b}$ to wektor oczekiwanej liczby przedmiotów w bloku.
                \item $\verb|is_aggregating|$ to wartość binarną informującą o tym czy blok jest \textit{agregujący}.
...
\section{Algorytm ewolucyjny}
    ...
    \subsection{Generowanie populacji początkowej}\label{subsection:generowanie_populacji}
        ...
        \begin{enumerate}
            ...
            \[ \forall d \in \left\{1, 2, 3, 4, 5\right\}: a_d' = \bigwedge_{t \in \underset{w_i \in L_i}{\bigcup}t_i} a_{t,d} \]
    ...
    \subsection{Przystosowanie}\label{subsection:przystosowanie}
        ...
        \subsubsection{Normalizacja i wagi}
            ...
            W celu rozwiązania tego problemu zastosowałem normalizację poprzez dzielenie każdej składowej przez odpowiednio $\left|\mathcal{T}\right|$ i $\mathfrak{G}$.
            ...
            \[ F_c = \frac{\alpha_T}{\left|\mathcal{T}\right|} F_{c_T} + \frac{\alpha_G}{\mathfrak{G}} F_{c_G}, \quad \alpha_T, \alpha_T \in \mathbb{R}^+ \]
