\Chapter{Problem układania planu lekcji i algorytm jego rozwiązania}\label{chapter:algorytm}
    ...
    \section{Sformułowanie problemu optymalizacyjnego}
        ...
        \begin{itemize}
            ...
            \item $\mathfrak{P}$ --- wielkość populacji w algorytmie ewolucyjnym
        \end{itemize}
        ...
        \subsection{Ograniczenia}\label{subsection:ograniczenia}
            ...
            \subsubsection{Ograniczenia fizyczne}
                \begin{itemize}
                    ...
                    \[ \forall i, j \in \left\{1, 2, \dots, \left|\mathcal{Z}\right|\right\}: \left(d_{z_i} = d_{z_j} \land h_{z_i} = h_{z_j} \land r_{z_i} \neq r_{z_j} \land i \neq j\right) \impliedby t_{z_i} \neq t_{z_j} \]
...
\section{Solver programowania liniowego z ograniczeniami}
    W trzecim etapie algorytmu wykorzystuję solver programowania liniowego z ograniczeniami (Constraint Programming --- CP), konkretnie implementację CP-SAT z pakietu \textit{Google OR-Tools}.
    ...
    W odpowiedzi na wyzwania napotkane w podejściu opisanym w podrozdziale~\ref{subsection:podejscie_zerojedynkowe} zdecydowałem się na wykorzystanie zmiennych interwałowych opisanych w dokumentacji OR-Tools~\cite{googleortoolsintervalvar}. 

    \textbf{Zalety podejścia CP:}
        \begin{itemize}
            ...
            \item \textbf{Specjalizowane ograniczenia}: Solver oferuje gotowe implementacje ograniczeń typowych dla harmonogramów --- nakładanie się.
            \item \textbf{Szybkość rozwiązywania}: Algorytmy propagacji ograniczeń efektywnie zawężają przestrzeń poszukiwań.
        \end{itemize}
