    ...
    \section{Sformułowanie problemu optymalizacyjnego}
        ...
        \subsection{Dane i szukane}
            ...
            \subsubsection{Wymagania główne}
                ...
                Aby łatwiej zrozumieć na czym polegają takie przypisania warto spojrzeć na dotychczasowy sposób przypisywania ilości godzin nauczycieli do klas (Rysunek~\ref{fig:excel_wymagania}) w liceum, które dostarczyło dane na potrzeby tej pracy.
        ...
        \subsection{Dekompozycja problemu}
            Proces tworzenia takich przypisań można rozbić na 3 etapy:adap
            \begin{enumerate}
            ...
            W mojej pracy zaadaptowałem i rozwinąłem tę fundamentalną ideę przedstawioną w podrozdziale~\ref{section:podejscie_iteracyjne}, dostosowując ją do specyfiki problemu układania planu lekcji.

            \subsubsection{Dekompozycja problemu}
                ...
                Pamięć operacyjna jest zawsze problemem w takich rozwiązaniach.
                Inspirując się podejściem przedstawionym w rozdziale 2~\ref{section:podejscie_iteracyjne}, zdecydowałem się na rozbiecie problemu na 3 mniejsze podproblemy gdzie każdy etap rozwiązuje wyraźnie wydzielony podproblem o mniejszej złożoności.
                % \[ \text{Problem pełny} \longrightarrow \text{Etap 1} \longrightarrow \text{Etap 2} \longrightarrow \text{Etap 3} \]

                gdzie:
                \begin{itemize}
                    \item \textbf{Etap 1 (algorytm zachłanny)}: Koncentruje się wyłącznie na ograniczeniach głównych i strukturalnych bloków lekcyjnych~(\ref{slownik_oznaczen}):
                    \[ \mathcal{L} = \text{Greedy}(\mathcal{W}, \mathcal{B}) \]
                    pomijając tymczasowo ograniczenia czasowe i przestrzenne.
                    \item \textbf{Etap 2 (algorytm ewolucyjny)}: Dodaje ograniczenia dostępności nauczycieli i równomierności rozkładu:
                    \[ S_{\text{best}} = \text{Evolutionary}(\mathcal{L}, V, A) \]
                    ale wciąż operuje na poziomie dni, a nie konkretnych slotów czasowych.
                    \item \textbf{Etap 3 (solver do problemów programowania z ograniczeniami)}: Przypisuje bloki lekcyjne do konkretnych slotów czasowych i sal:
                    \[ \mathcal{Z} = \text{Solver}(S_{\text{best}}, \mathcal{L}, \mathcal{R}) \]
                    procedura optymalizacyjna jest wykonywana niezależnie dla każdego dani.
                \end{itemize}
                

            \subsubsection{Rozwiązanie początkowe z prostymi ograniczeniami}
            ...
            \subsubsection{Hierarchiczna dekompozycja problemu}
                ...
                \begin{align*}
                    \text{Poziom 1:} & \quad \text{Struktura bloków lekcyjnych } \mathcal{L} \\
                    \text{Poziom 2:} & \quad \text{Rozkład tygodniowy } S_{\text{best}} \\
                    \text{Poziom 3:} & \quad \text{Przypisania szczegółowe } \mathcal{Z}
                \end{align*}
