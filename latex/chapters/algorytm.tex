    ...
    \section{Sformułowanie problemu optymalizacyjnego}
        ...
        \subsection{Dane i szukane}
            ...
            \subsubsection{Wymagania główne}
                ...
                \begin{figure}[H]
                    \centering
                    \includegraphics[width=0.7\textwidth]{images/excel_wymagania_1.png}
            ...
            \subsubsection{Bloki przedmiotów}\label{subject_blocks}
                ...
                \[
                    b_i \triangleq \left(\vec{S_{b,i}},\vec{C_{b,i}}, \vec{G_{b,i}}, \gamma_{b,i}, \epsilon_{b,i}\right) \in \mathcal{B}, \quad \forall i \in \left\{1, 2, \dots, \left|\mathcal{B}\right|\right\}: \begin{cases}
                    \vec{S_{b,i}} &\subset \mathcal{S} \\
                    \vec{C_{b,i}} &\subseteq  \mathcal{C} \\
                    \vec{G_{b,i}} &\in \mathbb{N}^{\left|\vec{S_{b,i}}\right|} \\
                ...
                \begin{itemize}
                    ...
                    \item $\vec{G_{b,i}}$ to wektor oczekiwanej liczby godzin przedmiotów w bloku,
                ...
                \end{itemize}
                Przykładowe bloki przedmiotów:
                \[ b_1 = \begin{cases}
                    \vec{S_b} &= \text{(język angielski, informatyka)}\\
                    \vec{C_b} &= \text{(IA, IB, IC)}\\
                    \vec{G_b} &= \left(1, 1\right)\\
                    \gamma &= 0\\
                    \epsilon &= 0\\
                \end{cases}, \quad b_2 = \begin{cases}
                    \vec{S_b} &= \text{(język angielski)}\\
                    \vec{C_b} &= \mathcal{C}\\
                    \vec{G_b} &= \left(2\right)\\
                    \gamma &= 0\\
                    \epsilon &= 0\\
                \end{cases} \]
        ...
        \subsection{Ograniczenia}\label{subsection:ograniczenia}
                ...
                \begin{samepage}
                    \textbf{Ograniczenia prawne}~\cite{karta_nauczyciela, prawo_oswiatowe, obwieszczenie}
                        \begin{itemize}
                            ...
                            \[ \forall d \in \left\{1, 2, \dots, 5\right\}, c \in \mathcal{C}, s \in \mathcal{S}: \left|\left\{z \in \mathcal{Z}: d_z = d \land c_z = c \land s_z = s\right\}\right| \leq 2 \]
            ...
            \subsubsection{Ograniczenia jakościowe}
                \begin{itemize}
                    \item Brak okienek dla uczniów. Nie istnieje taka para przypisań, dla których różnica między slotami czasowymi jest większa niż liczba wszystkich lekcji danego dnia,
                    \begin{align*}
                        \nexists i, j \in \left\{1, 2, \dots, \left|\mathcal{Z}\right|\right\}, c \in \mathcal{C}:\\
                        d_{z,i} = d_{z,j} \land c_{z,i} = c_{z,j} \land \left|h_{z,i} - h_{z,j}\right| > \left|\left\{z_k \in \mathcal{Z}: d_{z,k} = d_{z,i} \land c_{z,i}\right\}\right|
                    \end{align*}
                    ...
                    \begin{align*}
                        \forall w_i \in \mathcal{W}, \exists d \in \left\{1, 2, \dots, 5\right\}: \\ 
...
\section{Poprzednie podejścia}
    Aby w pełni zrozumieć mój wybór narzędzi warto szybko przytoczyć historię moich poprzednich podejść do rozwiązania tego problemu.
    ...
    \subsection{Grafowe sieci neuronowe}
        ...
        Początkowe rezultaty były bardzo obiecujące --- model już po kilkudziesięciu cyklach uczenia wykazywał zdolność do identyfikowania, które lekcji powinny być w blokach, a które wymagają rozdzielenia w celu uniknięcia kolizji.
...
\section{Dekompozycja problemu i wybór technik}
    W niniejszej pracy zaadaptowałem oraz rozwinąłem ideę przedstawioną w podrozdziale~\ref{section:podejscie_iteracyjne}, dostosowując ją do specyfiki mojego problemu.
    Zdecydowałem się na podział zadania na trzy mniejsze, kolejno realizowane etapy, z których każdy rozwiązuje wyraźnie wydzielony podproblem o niższej złożoności obliczeniowej niż problem kompletny.
    ...
    Zasadnicza innowacja przedstawionego przeze mnie podejścia polega na dekompozycji oryginalnego problemu na trzy podproblemy rozwiązywane sekwencyjnie odpowiednimi technikami, każda używająca adekwatnej techniki.
    Jest to rozwinięcie względem moich wcześniejszych koncepcji.
    ...
    \begin{itemize}
        ...
        \item \textbf{Przydział godzinowy}: Posiadając już zbiór takich bloków lekcyjnych macierz ich przypisań nazywamy przydziałem godzinowym.
        Każda kolumna takiej macierzy ma 5 wartości i reprezentuje liczbę godzin lekcyjnych przeprowadzoną każdego dnia.
        Przykładowo kolumna $\begin{bmatrix} 0 & 1 & 0 & 0 & 0 \end{bmatrix}^T$ reprezentuje jedną godzinę bloku we wtorek.
    \end{itemize}

    Dekompozycja prezentuje się następująco:
    \begin{itemize}
        \item Etap 1 --- \textbf{Problem grupowania}: \\
    ...
    Na rysunku~\ref{fig:dekompozycja_problemu} przedstawiłem diagram dekompozycji problemu.
    ...
    \begin{figure}[h]
        ...
        \begin{tikzpicture}[node distance=5cm]
            ...
            \node[block] (A) {Etap 1};
            \node[block, right of=A, yshift=1cm] (B) {Etap 2};
            \node[block, right of=B, yshift=-1cm] (C) {Etap 3};
            ...
            \draw[->] ([yshift=-8pt]A.east) -- node[above]{$\mathcal{L}$} ([yshift=-8pt]C.west);
            
            \draw[->] (B.east) -- node[above]{$S_{\text{best}}$} ([yshift=8pt]C.west);
        ...
        \end{tikzpicture}
        \caption{Dekompozycja algorytmu}\label{fig:dekompozycja_problemu}
    \end{figure}
    ...
    Trzeci etap sprowadza się do problemu harmonogramowania, który jak omówiłem wcześniej można przekształcić do postaci opisanej w podrozdziale~\ref{section:jssp}.
    ...
    Metoda ta nie wymaga projektowania własnego algorytmu rozwiązującego, co znacząco upraszcza proces implementacji, a jednocześnie gwarantuje wysoką jakość otrzymanych harmonogramów.
    W szczególności wykorzystałem zmienne przedziałowe, które ułatwiają modelowanie złożonych zależności czasowych i ograniczeń między lekcjami.
    ...
    \begin{figure}[h]
        ...
        \begin{tikzpicture}[node distance=5cm]
            ...
            \node[block, right of=A, yshift=1cm] (B) {\shortstack[c]{Algorytm\\ewolucyjny}};
            \node[block, right of=B, yshift=-1cm] (C) {\shortstack[c]{Programowanie\\liniowe}};
            ...
            \draw[->] (R3.south) -- (C.north);
        \end{tikzpicture}
        \caption{Struktura algorytmu}\label{fig:struktura_algorytmu_techniki}
    \end{figure}

    \subsection{Problem grupowania}
        \subsubsection{Bloki lekcyjne}
            ...
            \end{itemize}
            Z analizy wynika, że wszystkie zajęcia poza jedną lekcją języka niemieckiego mogą być prowadzone równocześnie.
        ...
        \subsubsection{Przestrzeń decyzyjna}
            ...
            Podobnie jak w pracy omówionej w podrozdziale~\ref{section:podejscie_iteracyjne}, gdzie autorzy zmniejszali złożoność przez agregację ograniczeń, moje podejście wykorzystuje bloki lekcyjne do scalenia wielu ograniczeń, co istotnie redukuje przestrzeń decyzyjną na dalszych etapach rozwiązania.
        ...
        \subsubsection{Funkcja celu}
            ...
            \[ F = \left|\left\{L \in \mathcal{L}: \left|L\right| > 1\right\}\right| \]
            ...
            W ten sposób gwarantujemy akceptowalną wartość funkcji celu.
    ...
    \subsection{Problem alokacji}
        ...
        \subsubsection{Funkcja celu}
            ...
            \quad
            \beta_{c,d} = \sum_{\substack{L_i \in \mathcal{L} \\ c\in\left\{c_{w,j}:\ w_j \in L_i\right\}}} s_{d,i} \]
            ...
            \end{equation}
            Analogicznie można definiować funkcję celu dla nauczycieli.

    \subsection{Problem harmonogramowania}
        Trzeci etap, podobnie jak poprzedni, cechuje się dużą przestrzenią decyzyjną.
        Istnieje wiele wykonalnych planów, z których część powoduje liczne okienka w harmonogramach nauczycieli.
        Z tego powodu zdecydowałem się zastosować programowanie liniowe z ograniczeniami (ang. \textit{Constraint Programming}, CP), które w praktyce dostarcza wysokiej jakości rozwiązania bez potrzeby projektowania własnego algorytmu przeszukiwania.

        Aby zapewnić poprawność i przejrzystość modelu, sformułowałem problem harmonogramowania jako specyficzny wariant JSSP z wykorzystaniem zmiennych przedziałowych.
        Na rysunku~\ref{fig:intervals} przedstawiłem przykładową wizualizację rozwiązania JSSP.

        \begin{figure}[ht]
            \centering
            \includegraphics[width=0.9\textwidth]{images/interval_vars.png}
            \caption{Wizualizacja rozwiązania JSSP z wykorzystaniem zmiennych przedziałowych~\cite{googleortoolsjssp}}\label{fig:intervals}
        \end{figure}

        Podejście oparte na przedziałach oferuje większą elastyczność niż model binarny z podrozdziału~\ref{subsection:podejscie_zerojedynkowe}, a jednocześnie upraszcza definicję kluczowych ograniczeń.
        ...
        \subsubsection{Zmienne decyzyjne i ograniczenia}~\label{subsubsection:zmienne_decyzyjne_ograniczenia}
            ...
            Dla każdego bloku lekcyjnego $L_i$, dla którego $s_{d,i} > 0$, wprowadzam zmienną przedziałową
            ...
            gdzie $\boldsymbol{s}_i$ i $\boldsymbol{e}_i$ oznaczają odpowiednio slot rozpoczęcia i zakończenia przedziału dla bloku $L_i$.

            Ograniczenia nienakładania się, implementuję przy użyciu funkcji \verb|AddNoOverlap|~\cite{googleortoolsoverlap}.
            Funkcja przyjmuje zbiór przedziałów i generuje wszystkie potrzebne ograniczenia bez konieczności ręcznego ich definiowania.
            Operację tę wykonuję niezależnie dla zbiorów przedziałów powiązanych z każdą klasą i każdym nauczycielem.

            Kolejną zaletą przedziałów jest możliwość użycia ich wariantu opcjonalnego do przypisywania sal.
            Dla każdej dopuszczalnej kombinacji (blok $L_i$, nauczyciel $t$, przedmiot $s$, sala $r$) definiuję opcjonalny przedział
            \[ O_{i,t,s,r} = (\boldsymbol{s}_i, s_{d,i}, \boldsymbol{e}_i, p_{i,t,s,r}), \quad p_{i,t,s,r} \in \{0,1\} \]
            gdzie rozważam wyłącznie kombinacje spełniające wymagania (np. blok z matematyką, nauczyciel matematyki, matematyka, sala do matematyki), co eliminuje niemożliwe kombinacje (analogicznie jak w podejściu z podrozdziału ~\ref{section:podejscie_iteracyjne}).
            ...
            Analogicznie do zwykłych przedziałów, nienakładanie dla sal tworzę poprzez \verb|AddNoOverlap|, przekazując zbiór wszystkich opcjonalnych przedziałów dla danej sali i powtarzając tę operację dla każdego $r \in \mathcal{R}$.
        ...
        \subsubsection{Funkcja celu}
            ...
            \[
                ...
                \end{cases}
            \]
            Wielkość $\Delta_t$ reprezentuje czas obecności nauczyciela w szkole (od początku pierwszego do końca ostatniego przedziału).
...
\section{Algorytm zachłanny}
    ...
    \subsection{Działanie}
        ...
        \noindent
        Procedurę generowania bloków można podzielić na pięć faz:
        \begin{itemize}
            \item \textbf{Tworzenie grup wieloklasowych} --- grupowanie wymagań głównych dotyczących tego samego przedmiotu dla różnych klas, przykładowo WF dla klasy IIIA, IIIC, IIID.
            \item \textbf{Agregacja bloków} --- łączenie powstałych bloków międzyklasowych z innymi wymaganiami, które mogą odbywać się równolegle, przykładowo wyżej wspomniany język rozyjski oraz język niemiecki dla klasy IIIA. 
            \item \textbf{Tworzenie bloków pojedynczych} --- formowanie bloków wewnątrz pojedynczych klas, przykładowo podział wychowania fizycznego na grupy chłopięce i dziewczęce.
            \item \textbf{Tworzenie bloków jednostkowych} --- utworzenie jednoelementowych zbiorów dla pozostałych wymagań, niezgrupowanych w poprzednich fazach, przykładowo lekcja języka polskiego w której musi uczestniczyć cała klasa.
            \item \textbf{Przydział godzin lekcyjnych do bloków} --- rozdzielenie godzin z wymagań głównych do bloków tak, aby każdy blok w obrębie klasy dostał przynajmniej jedną godzinę.
        \end{itemize}
        Szczegółowy opis działania przedstawiłem w pseudokodzie~\ref{alg:generowanie_blokow}.

        
        \begin{algorithm}[p]
            \caption{Generowanie bloków lekcyjnych z wymagań}\label{alg:generowanie_blokow}
            
            \Input{Zbiory wymagań $\mathcal{W}$, bloków $\mathcal{B}_{\text{single}}$, $\mathcal{B}_{\text{multi}}$, $\mathcal{B}_{\text{agg}}$, zbiór klas $\mathcal{C}$}
            ...
            \While{$\exists$ \textnormal{nie w pełni wykorzystane wymagania w blokach pojedynczych}}{
                \For{\textnormal{każdego bloku pojedynczego} $\mathcal{W}' \in \mathcal{L}$}{
                    \If{\textnormal{można zwiększyć liczbę godzin}}{
                        ...
                        \For{\textnormal{każdego} $w \in \mathcal{W}'$}{
                            $U(w) \gets U(w) + 1$ \;
                        }
                    }
                }
    ...
    \subsection{Przykładowy wynik}
        ...
        Pierwszym elementem w $\mathcal{L}$ jest komparatywnie duży, wieloklasowy blok lekcyjny powstały w fazie agregacji.
...
\section{Algorytm ewolucyjny}
    ...
    \begin{center}
        \begin{algorithm}[H]
            \caption{Ogólny schemat algorytmu ewolucyjnego~\cite{eiben2015evolutionary}}
    ...
    \subsection{Kodowanie i ograniczenia twarde}\label{subsection:kodowanie}
        Każdy osobnik w populacji jest reprezentowany przez macierz przydziałów $S$ przyjętą w definicji oznaczenia~(\ref{eq:specimen}).

        Tak przyjęta reprezentacja umożliwia prostą weryfikację następujących ograniczeń:
        \begin{itemize}
            \item Suma przypisań $i$-tego bloku musi być równa $v_i$. Zostało to opisane w warunku~(\ref{eq:specimen_wymaganie_glowne}).
            \item Maksymalna liczba godzin bloku w pojedynczym dniu nie może być większa niż 2.
    ...
    \subsection{Generowanie populacji początkowej}\label{subsection:generowanie_populacji}
        ...
        Przez $T_i = \{t_{w,j} : w_j \in L_i\}$ oznaczyłem zbiór nauczycieli prowadzących zajęcia w bloku $L_i$.
    ...
    \newpage
    \subsection{Przystosowanie}\label{subsction:przystosowanie}
        Ocena jakości osobników odbywa się na podstawie dwóch wzkaźników:
        \begin{itemize}
            \item wskaźnika równomierności obciążeń dydaktycznych lekcyjnych klas,
            \item wskaźnika równomierności obciążeń dydaktycznych pracy nauczycieli.
        \end{itemize}
        ...
        \subsubsection{Normalizacja i wagi}
            ...
            W praktyce, wymagania równomiernego rozkładu dla każdej klasy są ważniejsze niż równomierny rozkład obciążeń nauczycieli.
    ...
    \subsection{Krzyżowanie}\label{subsection:krzyzowanie}
        ...
        Z uwagi na dwie składowe funkcji przystosowania zdecydowałem się na zaimplementowanie dwóch niezależnych metod krzyżowania osobników widocznych w pseudokodach~\ref{alg:krzyzowanie_ocena_nauczyciel} oraz~\ref{alg:krzyzowanie_ocena_klas}.
    ...
    \subsection{Mutacja}\label{subsection:mutacja}
        ...
        \begin{samepage}
            ...
            \begin{enumerate}
                ...
                \item Dla każdej wybranej kolumny $j$ wylosuj nowy przydział zgodnie z procedurą opisaną w punkcie~\ref{subsection:generowanie_populacji}.
            \end{enumerate}
        \end{samepage}
    ...
    \subsection{Przykładowy wynik}\label{subsection:przykladowy_ewolucyjny}
        ...
        Dobór parametrów wynika z faktu, że w znacznie większym stopniu zależy nam na zapewnieniu komfortowego rozkładu obciążenia dydaktycznego uczniom niż nauczycielom.

        Na rysunku~\ref{fig:wyniki_ewolucyjny} przedstawiłem wartości funkcji celu dla pięciu niezależnych uruchomień algorytmu genetycznego dla 25 pokoleń, przy następujących parametrach:
        \begin{itemize}
            ...
            \centering
                \begin{tikzpicture}
                    \begin{axis}[
                        ...
                        legend entries={Osobnik 1, Osobnik 2, Osobnik 3, Osobnik 4, Osobnik 5},
                    ...
                    \end{axis}
                \end{tikzpicture}
            \caption{Wykres wartości funkcji celu dla pięciu uruchomień algorytmu genetycznego}\label{fig:wyniki_ewolucyjny}
        \end{figure} 
        ...
        W praktyce uniemożliwia to uzyskiwanie dodatnich wartości funkcji celu, ponieważ algorytm nie ma wystarczającej liczby bloków, by spełnić wszystkie wymagania strukturalne konieczne do uzyskania dodatniego wyniku.

\section{Programowanie liniowe z ograniczeniami}\label{section:solver}
    ...
    \subsection{Zmienne decyzyjne}
        Zmienne decyzyjne są definiwane zgodnie z definicjami przedstawionymi w punkcie~\ref{subsubsection:zmienne_decyzyjne_ograniczenia}.
    ...
    \subsection{Ograniczenia}
        ...
        \subsubsection{Kolejność przedmiotów}
    ...
    \subsection{Funkcja celu}
        Funkcja celu jest definiwana zgodnie z definicją przedstawioną w punkcie~\ref{subsubsection:zmienne_decyzyjne_ograniczenia}.

    \newpage
    \subsection{Transformacja przedziałów na przypisania końcowe}
        Po znalezieniu rozwiązania przez solver CP-SAT, konieczne jest przekształcenie zmiennych przedziałowych na ostateczne przypisania w zbiorze $\mathcal{Z}$.
        Proces ten obejmuje ekstrakcję wartości zmiennych decyzyjnych i odwzorowanie na strukturę danych planu lekcji.
        ...
        \subsubsection{Odwzorowanie na strukturę lekcji}
    ...
    \subsection{Przykładowy wynik}
        Dla danych wejściowych przedstawionych w poprzednim przykładzie otryzmałem następujące przypisania:
        \[
            \mathcal{Z} = \left\{ 
                \begin{aligned}
        ...
        Wszystkie lekcje zostały przypisane zgodnie z określonymi ograniczeniami.
        Nauczyciele otrzymali zajęcia zgodne z wymaganiami głównymi, 
        a przedmioty, w szczególności informatyka oraz chemia, zostały przydzielone do odpowiednio wyposażonych sal dydaktycznych.
