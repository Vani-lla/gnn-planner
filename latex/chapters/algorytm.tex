...
\section{Algorytm ewolucyjny}
    ...
    \subsection{Przykładowe rezultaty}
        Dla danych udostępnionych przez szkołę jak pokazano wcześniej liczba bloków lekcyjnych wynosi $\left|L\right| = 302$, więc macierz najlepszego osobnika prezentuje się następująco:
        ...
        \begin{itemize}
            \item Sumy się zgadzają
            \item Czasy działania algorytmu 
            \item Wartości funkcji celu
            \begin{itemize}
                \item interpretacja, czyli dlaczego nie otrzymujemy dodatnej wartości
            \end{itemize}
            \item wyjaśnienei hiperparametrów
            \item Histogramy dla nauczycieli i uczniów
        \end{itemize}
...
\section{Solver programowania liniowego z ograniczeniami}\label{section:solver}
    ...
    \subsection{Rezultaty}
        \begin{itemize}
            \item Przykładowe przypisania
            \item Czas obliczeń dla rozwiązań optymalnych i nieoptymalnych, podania parametrów kompa
            \item Liczba okienek i porównanie
            \item Liczba zmiennych decyzyzjnych
            \item Liczba ograniczeń
        \end{itemize}