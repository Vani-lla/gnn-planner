\Chapter{Aplikacja}\label{chapter:website}
    ...
    \begin{itemize}
        \item \textbf{Specyfikacja wymagań} --- formalizuje oczekiwania wobec systemu, dzieląc je na funkcjonalne i niefunkcjonalne, a następnie konkretyzując za pomocą diagramów przypadków użycia i sekwencji.
        \item \textbf{Projekt systemu} --- przedstawia wysokopoziomową architekturę aplikacji, szczegółowy projekt bazy danych oraz makietę interfejsu użytkownika.
        \item \textbf{Implementacja} --- opisuje konkretny stos technologiczny, sposób realizacji poszczególnych komponentów oraz integrację algorytmu z aplikacją webową.
    \end{itemize}

\section{Specyfikacja wymagań}
    Pełny zestaw wymagań, wraz ze szczegółowymi opisami, został zwizualizowany w formie diagramu wymagań na rysunku \ref{fig:diagram_wymagan}.
    ...
    \subsection{Przypadki użycia}
        Diagram przypadków użycia przedstawiony na rysunku~\ref{fig:usecases} pozwala na zwizualizowanie wcześniej omawianych wymagań funkcjonalnych w formie graficznej, ukazując interakcje między użytkownikami a systemem.
        ...
        W niniejszej sekcji omawiam kluczowe przypadki użycia zilustrowane na diagramie w tabelach od~\ref{tab:use_case_1} do~\ref{tab:use_case_last}, obejmujące zarówno generowanie planów lekcji, ich przeglądanie, jak i zarządzanie danymi wejściowymi systemu.

        \begin{sidewaysfigure}[p]
            \centering
            \includegraphics[width=0.85\textwidth,height=0.85\textheight,keepaspectratio]{images/diagramy/Use Case Diagram1.png}
        ...
        \begin{table}[p]
            ...
            \end{tabular}
            \caption{Przypadek użycia UC01}\label{tab:use_case_1}
        \end{table}
        ...
        \begin{table}[p]
            ...
            \end{tabular}
            \caption{Przypadek użycia UC12}\label{tab:use_case_last}
        \end{table}
    \clearpage
    ...
    \subsection{Diagramy sekwencji}
        ...
        \subsubsection{Diagram sekwencji generowania planu lekcji}
            ...
            Diagram na rysunku~\ref{fig:diag_sekw_1} przedstawia minimalny, konieczny zestaw komunikatów prowadzących do wywołania algorytmu optymalizacyjnego i zwrócenia użytkownikowi gotowego planu.
            \begin{figure}[H]
                \centering
                \includegraphics[width=1.0\textwidth]{images/diagramy/Sequence Diagram1.png}
                \caption{Diagram sekwencji generowania planu lekcji}\label{fig:diag_sekw_1}
            \end{figure}
        ...
        \subsubsection{Diagram sekwencji przeglądania planów lekcji}
            ...
            Proces został zilustrowany na diagramie~\ref{fig:diag_sekw_2}
            \begin{figure}[H]
                \centering
                \includegraphics[width=0.9\textwidth]{images/diagramy/Sequence Diagram2.png}
                \caption{Diagram sekwencji dla przypadku użycia oglądania planu lekcji}\label{fig:diag_sekw_2}
            \end{figure}

        \subsubsection{Diagram sekwencji wprowadzania i edycji danych}
            Trzeci scenariusz obejmuje wprowadzanie lub edycję danych, takich jak wymagania główne i został zaprezentowany na rysunku~\ref{fig:diag_sekw_3}.
            ...
            \begin{figure}[H]
                \centering
                \includegraphics[width=0.9\textwidth]{images/diagramy/Sequence Diagram3.png}
                \caption{Diagram sekwencji edycji bądź wprowadzania wymagań głównych}\label{fig:diag_sekw_3}
            \end{figure}
...
\section{Projekt}
    \subsection{Architektura aplikacji}
        Wybór odpowiedniej architektury aplikacji jest kluczowy dla zapewnienia jej skalowalności, elastyczności i możliwości dalszego rozwoju.
        ...
        \begin{itemize}
            ...
            \item \textbf{Bazę danych} --- przechowywuje wszystkie zasoby, ograniczenia oraz plany lekcji w ustrukturyzowany sposób.
        ...
        \begin{figure}[H]
            ...
            \begin{tikzpicture}[>=stealth, node distance=2cm]
                ...
                % Connections
                \draw[<->, very thick] (user.east) -- (gui.west) node[midway, above]{};
                \draw[<->, very thick] (gui.east) -- (api.west) node[midway, above]{};
                \draw[<->, very thick] (api.south) -- (db.north) node[midway, right]{};
                \draw[<->, very thick] (api.south west) -- (opt.north east) node[midway, above]{};
            \end{tikzpicture}
    ...
    \subsection{Projekt struktury bazy danych}\label{subsection:projekt_bazy_danych}
        ...
        \begin{itemize}
            \item tabele podstawowe,
            \item tabele wymagań, 
            \item tabele agregujące,
            \item tabele wyników.
        \end{itemize}

        Pełny diagram bazy danych znajduje się na rysunku~\ref{fig:baza_danych}, a omówienie kolumn tabel znajduje się w tabeli~\ref{tab:baza_danych}.
        ...
        \subsubsection{Tabele podstawowe}
            ...
            Relacje między nimi przedstawiłem w tabeli~\ref{tab:relacje_baza_1}, a ich diagram na rysunku~\ref{fig:diagram_baza_1}.

            \begin{table}[H]
                \centering
                \caption{Tabele podstawowe zasobów szkolnych}\label{tab:relacje_baza_1}
                \begin{tabular}{|r|l|}
                    \hline
            ...
            \begin{figure}[!th]
                ...
                \caption{Diagram tabel podstawowych w bazie danych}\label{fig:diagram_baza_1}
            \end{figure}
            \begin{figure}[!th]
                \centering
                \includegraphics[width=0.8\textwidth]{images/diagramy/diagram_db_wymagan.png}
                \caption{Diagram tabel podstawowych i wymagań w bazie danych}\label{fig:diagram_baza_1}
            \end{figure}
        ...
        \subsubsection{Tabele wymagań}
            ...
            Relacje tych tabel przedstawiłem w tabeli~\ref{tab:relacje_baza_2}, a ich diagram na rysunku~\ref{fig:diagram_baza_2}.

            \begin{table}[H]
                \centering
                \caption{Tabele definiujące wymagania i ograniczenia}\label{tab:relacje_baza_2}
                \begin{tabular}{|r|l|}
                    \hline
        ...
        \subsubsection{Zbiory danych}
            ...
            Ich relacje przedstawiłem w tabeli~\ref{tab:relacje_baza_3}

            \begin{table}[H]
                \centering
                \caption{Tabele zbiorów zasobów}\label{tab:relacje_baza_3}
                \begin{tabular}{|r|l|}
                    \hline
                    \textbf{Tabela} & \textbf{Relacja} \\
                    \hline
            ...
            \newpage
            W przypadku sal przyjąłem relację jeden-do-wielu, co wynika z całkowitej niezmienności tej puli zasobów pomiędzy okresami szkolnymi.
            ...
            Jej relacje przedstawiłem w tabeli~\ref{tab:relacje_baza_4}.

            \begin{table}[H]
                \centering
                \caption{Tabela requirement\_set}\label{tab:relacje_baza_4}
                \begin{tabular}{|r|l|}
                    \hline
                    \textbf{Tabela} & \textbf{Relacja} \\
                    \hline
                    \multirow{3}{*}{\texttt{requirement\_set}} & 1:N z \texttt{requirement}, \texttt{teacher\_availability}, \texttt{subject\_block} \\
                    & N:1 z \texttt{subject\_pool}, \texttt{student\_group\_pool}, \\
                    & \ \ \ \ \ \ \ \ \texttt{teacher\_pool} oraz \texttt{room\_pool} \\
                    \hline
                \end{tabular}
            \end{table} 
            ...
            \begin{itemize}
                ...
                \begin{itemize}
                    \item 1:N z tabelami \texttt{requirement}, \texttt{teacher\_availability}, \texttt{subject\_block}. \\
        ...
        \subsubsection{Tabele wyników}
            Relacje tabel przechowujących wygenerowane plany lekcji i ich przypisania zaprezentowałem w tabeli~\ref{tab:relacje_baza_5}.

            \begin{table}[H]
                \centering
                \caption{Tabele przechowujące wygenerowane plany lekcji}\label{tab:relacje_baza_5}
                \begin{tabular}{|l|l|}
                    \hline
                    \textbf{Tabela} & \textbf{Relacje} \\
                    \hline
        ...
        \begin{figure}[p]
            ...
            \caption{Diagram pełnego projektu bazy danych}\label{fig:baza_danych}
        \end{figure}

        \begin{table}[p]
            \centering
            \caption{Struktura najważniejszych tabel w bazie danych}\label{tab:baza_danych}
    ...
    \subsection{Projekt interfejsu}
        ...
        \subsubsection{Moduły wprowadzania danych}
            ...
            \begin{itemize}
                \item \textbf{Strony zasobów} umożliwiają zarządzanie czterema podstawowymi zasobami: przedmiotami, nauczycielami, klasami oraz salami.
...
\section{Implementacja}
    \subsection{Wybór narzędzi}
        \subsubsection{Interfejs webowy}
            Frontend to część aplikacji odpowiadająca za interfejs użytkownika i działająca po stronie przeglądarki internetowej.
            Framework frontendowy to zestaw narzędzi, bibliotek i konwencji programistycznych, które ułatwiają i przyspieszają tworzenie oprogramowania, zapewniając gotowe rozwiązania dla typowych problemów.
            ...
            Po trzecie, React oferuje responsywne zarządzanie stanem aplikacji, co umożliwia dynamiczną aktualizację widoków planów lekcji bez konieczności przeładowywania strony, zapewniając płynność interakcji użytkownika podczas przeglądania planów lekcji.
        ...
        \subsubsection{Warstwa serwerowa}
            ...
            Backend to część aplikacji działająca po stronie serwera, jest odpowiedzialna za logikę, komunikację z bazą danych oraz zapewnianie API dla frontendu.
            Stanowi on fundament aplikacji webowej, podczas gdy frontend odpowiada za prezentację danych użytkownikowi.
            Podobnie jak w przypadku frontendu istnieje wiele frameworków ułatwiających tworzenie tej części aplikacji.

            Wybór Django~\cite{django} jako frameworka backendowego został dokonany po analizie alternatywnego rozwiązania w Pythonie --- FastAPI~\cite{fast_api}.
        ...
        \subsubsection{Moduł optymalizacyjny}
            ...
            W trzecim etapie użyłem pakietu Google OR-Tools, który oferuje bogaty zestaw narzędzi do modelowania problemów programowania z ograniczeniami, w tym zmienne przedziałowe i ograniczenia typu \verb|NoOverlap|.
    ...
    \subsection{Implementacja bazy danych w Django}
        ...
        Każda tabela w systemie, z wyjątkiem automatycznie generowanych tabel dla relacji wiele-do-wielu, jest definiowana przez dedykowaną klasę~\cite{djangomodeldocs} w pliku \verb|models.py|. 
...
class Room(models.Model):
    ...
    compatible_subjects = models.ManyToManyField(Subject)
        ...
        Implementacja dla reszty tabel opisanych w punkcie~\ref{subsection:projekt_bazy_danych} jest analogiczna.

    \subsection{Implementacja API}
        Aby zrozumieć implementację API w Django należy znać parę pojęć:
        \begin{itemize}
            \item \textbf{Serializacja} (ang. \textit{Serializing}) to proces konwersji obiektów Django na format danych przesyłany przez API oraz konwersja danych przychodzących na obiekty Django.
            \item \textbf{Widok} (ang. \textit{View}) to komponent odpowiedzialny za przetwarzanie żądań HTTP i zwracanie odpowiedzi, implementujący logikę biznesową dla konkretnych punktów końcowych.
            \item \textbf{Punkt końcowy} (ang. \textit{Endpoint}) to unikalny adres URL w API, który odpowiada na żądania HTTP i udostępnia konkretną funkcjonalność.
        \end{itemize}
        ...
        Jedną z kluczowych zalet frameworka są widoki \verb|ModelViewSet|, które automatycznie generują kompletny zestaw punktów końcowych dla modelu na podstawie minimalnej konfiguracji, eliminując konieczność ręcznego implementowania rutynowych operacji.
        ...
        \begin{figure}[H]
            ...
            \caption{Automatycznie wygenerowana lista punktów końcowych stworzonych przy użyciu \mbox{ModelViewSet}}\label{fig:api_default_router}
        \end{figure}

        Na rysunku~\ref{fig:code2} przedstawiłem przykładową implementację punktów końcowych dotyczących tabeli \verb|room_pool| używając klast \verb|ModelViewSet|.
        \begin{figure}[H]
            \begin{lstlisting}[language=Python]
...
class RoomPoolViewSet(ModelViewSet):
    ...
    serializer_class = RoomPoolSerializer
            \end{lstlisting}
            \caption{Fragment kodu służący do implementacji punktów końcowych dla zbioru sal}\label{fig:code2}
        \end{figure}
    ...
    \subsection{Implementacja interfejsu webowego}
        ...
        \subsubsection{Strona główna}
            ...
            \subsubsection{Podgląd planów lekcji}
                ...
                \begin{figure}[H]
                    \centering
                    \includegraphics[width=0.9\textwidth]{images/strona/plan_klasa_1.png}
        ...
        \subsubsection{Widoki wprowadzania zasobów}
            ...
            Implementacje przedstawione na rysunkach~\ref{fig:add_subject},~\ref{fig:add_teacher},~\ref{fig:add_room} i~\ref{fig:add_class} spełniają wymagania funkcjonalne co do dodawania zasobów.
    ...
    \subsection{Integracja z algorytmem}
        ...
        Zaimplementowałem dedykowany punkt końcowy, który przyjmuje żądania typu POST zawierające parametry konfiguracyjne algorytmu, w tym liczbę generacji dla algorytmu ewolucyjnego oraz ID zbioru wymagań.\\
        Dalszy proces jest opisany na diagrame seksencji~\ref{fig:diag_sekw_1}
