\Chapter{Powiązane prace}\label{chapter:related}
    ...
    \begin{itemize}
        \item \textbf{Podejścia algorytmiczne i metodyczne} --- analiza pracy naukowej przedstawiającej zaawansowaną, iteracyjną metodę optymalizacji, która posłużyła jako kluczowa inspiracja architektoniczna.
        \item \textbf{Podstawy teoretyczne} --- formalne wprowadzenie do klasycznego problemu harmonogramowania zadań typu job-shop, który stanowi teoretyczną ramę dla problemu harmonogramowania i uzasadnia użycie technik programowania z ograniczeniami.
        \item \textbf{Rozwiązania praktyczne i komercyjne} --- szczegółowy przegląd wiodącego na rynku polskim systemu, służący zrozumieniu rzeczywistych wymagań, złożoności problemu oraz ustaleniu punktu odniesienia dla oceny praktyczności własnego rozwiązania.
    \end{itemize}
    ...
    \section{Inspiracja podejściem iteracyjnym}\label{section:podejscie_iteracyjne}
        ...
        Kluczowym elementem przedstawionej w pracy metody jest dekompozycja oryginalnego, złożonego problemu na szereg prostszych podproblemów, rozwiązywanych sekwencyjnie w procesie iteracyjnym.
            ...
            \subsubsection{Podstawowy model MIP}
                ...
                \begin{itemize}
                    \item zmienne $x_{l,h}$ określają czy zajęcia $l$ są przypisane do terminu $h$,
                    \item zmienne $y_{l,r}$ określają czy zajęcia $l$ są przypisane do sali $r$,
    ...
    \section{Problem harmonogramowania zadań typu job-shop}\label{section:jssp}
        ...
        Zmiennymi decyzyjnymi są czasy rozpoczęcia operacji $S_{ij} \in \mathbb{N}$. Definiujemy czas zakończenia operacji jako $C_{ij} = S_{ij} + p_{ij}$, a czas zakończenia zadania $J_i$ jako $C_i = C_{i n_i}$.
        ...
        \begin{itemize}
            \item \textbf{Ograniczenia kolejnościowe}: dla każdego zadania $J_i$ i $j=1,\dots,n_i-1$ musi zachodzić.
...
\section{Istniejące kompleksowe rozwiązania}
    ...
    \subsection{Plan lekcji Optivum}
        ...
        Podstawą do tworzenia planu są dane zaimportowane z arkusza organizacyjnego w procesie przedstawionym na rysunku~\ref{fig:import_arkusza}.
        ...
        \begin{figure}[!h]
            \centering
            \includegraphics[width=0.60\textwidth]{images/vulcan/import_arkusz.png}
            \caption{Zrzut ekranu importowania arkusza organizacyjnego do programu ,,Plan lekcji Optivum''~\cite{vulcanprzewodnik}}\label{fig:import_arkusza}
        \end{figure}

        W następnym etapie użytkownik definiuje zasoby lokalne zaczynając od sal oraz preferencji, co widać kolejno na rysunkach~\ref{fig:wprowadzanie_sal},~\ref{fig:sale_pref1} i~\ref{fig:sale_pref2}.
        Dla zajęć grupowych kluczowe jest ich poprawne zdefiniowanie.
        ...
        \begin{figure}[p]
            ...
            \caption{Zrzut ekranu wprowadzania sal do programu ,,Plan lekcji Optivum''~\cite{vulcanprzewodnik}}\label{fig:wprowadzanie_sal}
        \end{figure}
        
        \begin{figure}[p]
            \centering
            \includegraphics[width=0.65\textwidth]{images/vulcan/sale_pref2.png}
            \caption{Zrzut ekranu wprowadzania preferencji sal względem przedmiotów do programu ,,Plan lekcji Optivum''~\cite{vulcanprzewodnik}}\label{fig:sale_pref1}
        \end{figure}
        
        \begin{figure}[p]
            \centering
            \includegraphics[width=0.65\textwidth]{images/vulcan/sale_pref1.png}
            \caption{Zrzut ekranu wprowadzania preferencji sal względem nauczycieli do programu ,,Plan lekcji Optivum''~\cite{vulcanprzewodnik}}\label{fig:sale_pref2}
        \end{figure}

        Po wprowadzeniu sal oraz ich preferencji użytkownik jest poproszony o wprowadzenie ewentualnych podziałów na bloki, co obrazuje rysunek~\ref{fig:wprowadzanie_blokow}.
        ...
        \begin{figure}[H]
            \centering
            \includegraphics[width=0.55\textwidth]{images/vulcan/bloki_1.png}
            \caption{Zrzut ekranu wprowadzania bloków do programu ,,Plan lekcji Optivum''~\cite{vulcanprzewodnik}}\label{fig:wprowadzanie_blokow}
        \end{figure}
        
        \vspace*{-1em}
        W kolejnym etapie wprowadzane są terminy odbycia zajęć w poszczególnych klasach.
        Program na bieżąco wylicza ,,bilans gwiazdek'' przedstawiony na rysunku~\ref{fig:gwiazdki} --- różnicę między liczbą gwiazdek a minimalną liczbą potrzebną do ułożenia planu.
        ...
        \begin{figure}[p]
            \centering
            \includegraphics[width=0.6\textwidth]{images/vulcan/horyzont.png}
            \caption{Zrzut ekranu wprowadzania terminów zajęć dla poszczególnych klas do programu ,,Plan lekcji Optivum''~\cite{vulcanprzewodnik}}\label{fig:gwiazdki}
        \end{figure}
        
        Następnie definiuje się dostępność nauczycieli w sposób przedstawiony na rysunku~\ref{fig:dostepnosc_nauczycieli_vulcan}.
        ...
        \begin{figure}[p]
            \centering
            \includegraphics[width=0.6\textwidth]{images/vulcan/dostepnosc_nauczycieli.png}
            \caption{Zrzut ekranu wprowadzania dostępności nauczycieli do programu ,,Plan lekcji Optivum''~\cite{vulcanprzewodnik}}\label{fig:dostepnosc_nauczycieli_vulcan}
        \end{figure}
        ...
        Rysunek~\ref{fig:trudne_lekcje} obrazuje wprowadzenie przykładowych takich lekcji.

        \begin{figure}[p]
            \centering
            \includegraphics[width=0.6\textwidth]{images/vulcan/plan_trudne_1.png}
            \caption{Zrzut ekranu wprowadzania ,,trudnych'' lekcji do programu ,,Plan lekcji Optivum''~\cite{vulcanprzewodnik}}\label{fig:trudne_lekcje}
        \end{figure}

        Dalej uruchamia się automat dla całego planu.
        Jeśli automat nie poradzi sobie z ułożeniem wszystkich lekcji, należy przeanalizować nieułożone lekcje i złagodzić parametry.
        Czasami pomaga kilkakrotne wykonanie minimalizacji okienek i układania całego planu.

        Narzędzie ,,Plan lekcji Optivum'' jest obszernym narzędziem oferującym wiele możliwości. 
